...
class LoggingButton extends Component {
    ...
    constructor(props) {
        super(props);
        this.state={
            request: "http://localhost:7070/poatenumergi/loginRA/"
        }
    ...
    handleClick = () => {
        if (this.validateLoginData(this.props)) {
            if(!this.props.checked) {
                ...
                this.setState({request:"http://localhost:7070/poatenumergi/loginCustomer/"},this.fetchRequest)
            }
            else{
                this.setState({request:"http://localhost:7070/poatenumergi/loginRA/"},this.fetchRequest)
            }
...
function RegisterButton(props) {
    ...
    const validateRegisterData = () => {
        if (username==="") {
            seterrorMessageForLabel("You must provide a username.");
            return false;
        }
        if (password==="") {
            seterrorMessageForLabel("You must provide a password.");
            return false;
        }
        if (email==="") {
            seterrorMessageForLabel("You must provide an email address.");
            return false;
        }
        if (!isChecked) {

            if (firstname==="") {
                seterrorMessageForLabel("Please enter your first name.");
                return false;
            }
            if (lastname==="") {
                seterrorMessageForLabel("Please enter your last name.");
                return false;
            }
        }
    ...
    const handleSubmit = async (e) => {
        ...
        if (validateRegisterData()) {
            if(isChecked){
                ...
                let response = await fetch("http://localhost:7070/poatenumergi/addRA", {
                    method: 'POST',
                    headers: {
                ...
                let response = await fetch("http://localhost:7070/poatenumergi/addCustomer", {
                    method: 'POST',
                    headers: {
                        "Content-Type": "application/json",
                        "Accept": "application/json"
        ...
        <>
            ...
            <Modal className="ModalStyle" style={{opacity:1}} show={show} onShow={initializeModal} onHide={handleClose}
                ...
                <Modal.Header closeButton>
                    <Modal.Title>One account.<br></br>Endless possibilities.</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Container>
                        <Form>
                            <Form.Group className="mb-3" controlId="formBasicCheckbox">
                                <Form.Check type="checkbox" label="I am a restaurant administrator" onClick={handleChecked} className="HomepageLabel"/>
                            </Form.Group>
                            ...
                            {isChecked===false&&
                                <>
                                    <Form.Group as={Row} className="mb-3" controlId="formHorizontalFirstName">
                                        ...
                                        <Col sm={2}>
                                            <Form.Control type="firstname" placeholder="First_name" onChange={(e)=>setFirstName(e.target.value)}/>
                                        </Col>
                                    </Form.Group>
                                    <Form.Group as={Row} className="mb-3" controlId="formHorizontalLastName">

                                        <Col sm={2}>
                                            <Form.Control type="lastname" placeholder="Last_name" onChange={(e)=>setLastName(e.target.value)}/>
                                        </Col>
                                    </Form.Group>
                                </>
                            }
        ...
        <Form>
            <Form.Group as={Row} className="mb-3" controlId="formHorizontalUsername">
                ...
                <Form.Group as={Col} className="lg-3" controlId="formCheckBox">
                    <Col sm={4}>
                        <Form.Check type="checkbox" label="I am a restaurant administrator" onChange={handleChecked} className="HomepageLabel"/>
                    </Col>
                </Form.Group>
            </Form.Group>
